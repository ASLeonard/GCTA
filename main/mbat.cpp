...
void gcta::mbat_calcu_lambda(std::vector<int> &snp_indx, Eigen::MatrixXf &rval, Eigen::VectorXd &eigenval, int &snp_count, double sbat_ld_cutoff, std::vector<int> &sub_indx)
{
...
void gcta::svdDecomposition( Eigen::MatrixXf &X,double &prop, int &eigenvalueNum, Eigen::VectorXd &eigenvalueUsed,Eigen::MatrixXd &U_prop){
    Eigen::VectorXd cumsumNonNeg; // cumulative sums of non-negative values
    double sumNonNeg;

    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> eigensolver(X);
    Eigen::VectorXd eigenVal = eigensolver.eigenvalues().cast<double>();
    Eigen::MatrixXd eigenVec = eigensolver.eigenvectors().cast<double>();
...
void gcta::mbat_gene(std::string mbat_sAssoc_file, std::string mbat_gAnno_file, int mbat_wind, double mbat_svd_gamma, double sbat_ld_cutoff, bool mbat_write_snpset, bool GC, double GC_val,bool mbat_print_all_p)
{
    int i = 0, j = 0;
    std::vector<int> snp_chr;
    ...
    std::cout << " StatFunc::pchisq(1e100, 1):" << std::to_string( StatFunc::pchisq(1e100, 1)) << " "<< StatFunc::pchisq(1e100, 1) << std::endl;
    ...
    chr_begin_snp.insert(std::pair<int, std::string>(_chr[_include[0]], _snp_name[_include[0]]  ));
    for (i = 1; i < snp_num; i++) {
        if (_chr[_include[i]] != _chr[ _include[i - 1]]) {
            chr_begin_snp.insert(std::pair<int, std::string>(_chr[_include[i]], _snp_name[_include[i]] ));
            chr_end_snp.insert(std::pair<int, std::string>(_chr[_include[i-1]], _snp_name[_include[i -1]] ));
        }
    }
    chr_end_snp.insert(std::pair<int, std::string>(_chr[_include[snp_num -1]], _snp_name[_include[snp_num - 1]]));
    ...
    /////////////////////////////////////////
    LOGGER << "Mapping the physical positions of genes to SNP data (gene boundaries: " << mbat_wind / 1000 << "Kb away from UTRs) ..." << std::endl;

    std::vector<int> gene_mapped_idx;
    ...
    for (j = 0; j < mapped; j++) {
        ...
        if(snp_num_in_gene[j] == 1) {
            ...
            Eigen::MatrixXd Uprop_z = U_prop.transpose() * zscore_in_gene; 
            Eigen::MatrixXd lambda_prop_diag_inv = eigenvalueUsed.asDiagonal().inverse();        
...
void gcta::mbat(std::string mbat_sAssoc_file, std::string snpset_file, double mbat_svd_gamma, double sbat_ld_cutoff, bool mbat_write_snpset, bool GC, double GC_val,bool mbat_print_all_p)
{
    ...
    for (i = 0; i < set_num; i++) {
        ...
        if(snp_num_in_set[i] == 1) {
            ...
            Eigen::MatrixXd Uprop_z;
            Eigen::MatrixXd lambda_prop_diag_inv;
            svdDecomposition(rval, mbat_svd_gamma,eigenvalueNum_mBAT[i],eigenvalueUsed, U_prop);
            zscore_in_set.resize(snp_indx.size());
            // min_snp_pval[j]=2;
            // min_snp_name[j]="na";
